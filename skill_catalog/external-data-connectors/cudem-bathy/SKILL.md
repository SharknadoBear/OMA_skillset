---
name: cudem-bathy
description: Fetch and inspect NOAA CUDEM-first bathymetry/topobathymetry with optional NBS BlueTopo, CRM, and ETOPO fallback. Use when Codex needs a generic bathymetry Python toolbox for source indexing, estimate-first tile planning, local-or-Kestrel routing, mosaics or point sampling, coverage diagnostics, and health plots.
---
...
- Primary source: NOAA CUDEM, NBS BlueTopo, NOAA CRM, and ETOPO bathymetry/topobathymetry sources.
- Toolbox focus: bathymetry indexing, tile selection, mosaics, sampled depths, source coverage, and datum/provenance records.
- Main packaged scripts:
- `scripts/build_cudem_index.py`
- `scripts/build_bathy_source_index.py`
- `scripts/fetch_cudem_bathy.py`
- `scripts/fetch_bathy_sources.py`
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/cudem-bathy/<run-id>`;
