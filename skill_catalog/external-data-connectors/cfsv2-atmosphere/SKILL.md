---
name: cfsv2-atmosphere
description: Fetch and inspect CFSv2 atmospheric forcing from THREDDS/OPeNDAP for regional ocean, lake, and coastal modeling workflows. Use when Codex needs estimate-first CFSv2 download planning, local-or-Kestrel routing, regular-grid cache products, and health checks for gridded atmospheric data.
---
...
- Primary source: NCEP CFSv2 atmospheric forcing through HYCOM THREDDS/OPeNDAP.
- Toolbox focus: wind, pressure, flux, and regular-grid atmospheric forcing subsets.
- Main packaged script: `scripts/cfsv2_fetcher.py`.
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/<username>/oma_external_data_connectors/cfsv2-atmosphere/<run-id>`;
...
```powershell
python scripts/cfsv2_fetcher.py window `
  --start 2019-07-01T00:00:00Z --end 2019-07-01T06:00:00Z `
  --subdataset uv-10m --lon-min 283 --lon-max 288 --lat-min 36 --lat-max 41 `
  --output runs/smoke/uv-10m.nc --report runs/smoke/fetch_report.json
```
...
- Use canonical subdataset `dlwsfc`; accept historical `dlwflx` only as an alias. The field inside the file remains `dlwflx`.
- Treat HYCOM CFSv2 `airprs` as a pressure departure from the HYCOM 1000 hPa base. Convert it explicitly with `cfsv2_airprs_to_absolute_pa`; do not label raw `airprs` as absolute pressure.
- Use the public `fetch_cfsv2_window` API or its `window` CLI for bounded work. Keep `fetch_cfsv2_year`, `fetch_wind_year`, and `fetch_pressure_year` for backward compatibility only.
...
- Exercise `cfsv2_fetcher.py --help`, `window --help`, and `pressure-to-pa --help`.
