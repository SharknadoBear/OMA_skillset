---
name: hycom-obc-hermes
description: Execute explicit HYCOM OPeNDAP request manifests for unattended gridded, point, profile, and boundary workflows. Use when Codex or Hermes needs a noninteractive generic HYCOM Python toolbox with estimate-first planning, local-or-Kestrel routing, run evidence, and health checks.
---
...
- Toolbox focus: unattended HYCOM request execution, explicit manifests, chunk plans, and failure evidence.
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/hycom-obc-hermes/<run-id>`;
