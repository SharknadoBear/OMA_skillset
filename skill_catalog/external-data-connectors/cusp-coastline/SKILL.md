---
name: cusp-coastline
description: Fetch and inspect NOAA NGS CUSP shoreline vectors with optional OSM Overpass fallback. Use when Codex needs a generic coastline Python toolbox for source indexing, estimate-first download planning, local-or-Kestrel routing, vector clipping, provenance-preserving merge, map diagnostics, and health checks.
---
...
- Primary source: NOAA NGS Continually Updated Shoreline Product with optional OSM Overpass fallback.
- Toolbox focus: shoreline vector downloads, bbox clipping, provenance-preserving fallback merge, and visual QA.
- Main packaged scripts:
- `scripts/build_cusp_region_index.py`
- `scripts/fetch_cusp_coastline.py`
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/cusp-coastline/<run-id>`;
