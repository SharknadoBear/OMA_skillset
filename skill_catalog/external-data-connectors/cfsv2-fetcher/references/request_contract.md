...
Legacy subdataset APIs and `window` remain available. Direct mappings include wind, pressure, downward fluxes, stress, precipitation, and surface temperature. Derived `wndspd` and composite `TaqaQrQp` remain deprecated HYCOM-only paths and are not presented as native NCEI equivalents.
