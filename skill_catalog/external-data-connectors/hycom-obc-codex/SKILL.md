---
name: hycom-obc-codex
description: Plan, test, debug, and extend HYCOM OPeNDAP downloads for gridded, point, profile, and boundary workflows. Use when Codex needs an interactive generic HYCOM Python toolbox with estimate-first planning, chunk review, local-or-Kestrel routing, health checks, and optional legacy open-boundary compatibility.
---
...
- Primary source: HYCOM THREDDS/OPeNDAP gridded ocean fields.
- Toolbox focus: interactive HYCOM request planning, chunking, gridded subsets, point extraction, and boundary-oriented interpolation helpers.
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/hycom-obc-codex/<run-id>`;
