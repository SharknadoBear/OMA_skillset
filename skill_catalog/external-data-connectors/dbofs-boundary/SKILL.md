---
name: dbofs-boundary
description: Fetch and inspect NOAA DBOFS hydrodynamic fields for regional boundary, initial-condition, and station workflows. Use when Codex needs a generic Python toolbox for DBOFS NetCDF estimate planning, local-or-Kestrel routing, temporary downloads, field extraction, and downloaded-data health checks.
---
...
- Primary source: NOAA DBOFS archived hydrodynamic fields.
- Toolbox focus: DBOFS field download, extraction, interpolation-ready products, and cache cleanup.
- Main packaged scripts:
- `scripts/dbofs_fetcher.py`
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/dbofs-boundary/<run-id>`;
