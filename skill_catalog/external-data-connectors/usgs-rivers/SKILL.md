---
name: usgs-rivers
description: Fetch and inspect USGS NWIS streamflow observations and derived river time-series products. Use when Codex needs a generic Python toolbox for estimate-first river-data planning, API/cache fallback, hydrograph QA, simple tracer/load helpers, and downloaded-data health checks.
---
...
- Primary source: USGS NWIS river discharge observations.
- Toolbox focus: streamflow fetches, cache fallback, unit conversion, hydrograph summaries, and optional source apportionment.
- Main packaged scripts:
- `scripts/usgs_rivers_fetcher.py`
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/usgs-rivers/<run-id>`;
