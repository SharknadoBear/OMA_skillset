...
def _explicit_size_bytes(obj: Any) -> float | None:
    if isinstance(obj, list) and any(isinstance(item, dict) and "selected" in item for item in obj):
        selected_items = [
            item
            for item in obj
            if isinstance(item, dict)
            and str(item.get("selected", "")).strip().lower() in {"1", "true", "yes", "y", "selected"}
        ]
        obj = selected_items
