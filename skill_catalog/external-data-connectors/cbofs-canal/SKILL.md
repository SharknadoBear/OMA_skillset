---
name: cbofs-canal
description: Fetch and inspect NOAA CBOFS hydrodynamic fields for canal, station, and bounded model-input workflows. Use when Codex needs a generic Python toolbox for CBOFS NetCDF download planning, estimate-first routing, field extraction, cache handling, and downloaded-data health checks with optional legacy model compatibility.
---
...
- Primary source: NOAA CBOFS archived hydrodynamic fields.
- Toolbox focus: CBOFS station/canal extraction, temporary NetCDF download, and ROMS-grid variable sampling.
- Main packaged scripts:
- `scripts/cbofs_fetcher.py`
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/cbofs-canal/<run-id>`;
