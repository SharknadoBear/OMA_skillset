---
name: usseabed-sediment
description: Fetch and inspect USGS usSEABED tabular sediment products. Use when Codex needs a generic Python toolbox for estimate-first table downloads, local-or-Kestrel routing, spatial subsetting, sediment summaries, and health plots for point or tabular data.
---
...
- Primary source: USGS usSEABED tabular sediment data.
- Toolbox focus: CSV table downloads, sediment subsetting, grain-size summaries, and point/map diagnostics.
- Main packaged scripts:
- `scripts/usseabed_fetcher.py`
...
3. Use the estimate result to choose storage:
   ...
   - if the local disk does not satisfy that rule, use `$kestrel-hpc` and plan work under `/scratch/yhuang168/oma_external_data_connectors/usseabed-sediment/<run-id>`;
