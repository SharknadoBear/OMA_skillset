# CBOFS public AWS source contract

## Access and discovery

- Bucket: `noaa-nos-ofs-pds`; use anonymous ListObjectsV2 and HTTPS only.
- Discover both `cbofs/netcdf/YYYY/MM/DD/` and `cbofs/netcdf/YYYYMM/`.
...
- Reject non-cycle hours, native `n000`/`n007+`, native `f000`, station n/f
  objects, and field/regular-grid aggregate names.
...
- Approve only exact `cbofs/netcdf/YYYY/MM/DD/` or
  `cbofs/netcdf/YYYYMM/` keys whose directory date matches the filename run,
  and only the canonical NOAA S3 URL for that key. Require nonempty ETag and
  Last-Modified provenance before local routing.
...
missing ETag fails. Transfer accepts a reviewed plan only. Extraction from a
