...
def plot_gshhs_map(
    ...
    *,
    ...
    display_crs = plot_crs or 3857
    bbox_display = bbox_gdf.to_crs(display_crs)
    minx, miny, maxx, maxy = bbox_display.total_bounds
    ...
    if not land_gdf.empty:
        land_gdf.to_crs(display_crs).plot(ax=ax, facecolor="#eee6d6", edgecolor="#475569", linewidth=0.5, alpha=0.9)
    if not coastline_gdf.empty:
        coastline_gdf.to_crs(display_crs).plot(ax=ax, color="#0f172a", linewidth=0.75, alpha=0.9)
    bbox_display.boundary.plot(ax=ax, color="#dc2626", linewidth=1.2, linestyle="--", label="source footprint")
    if source_frame_gdf is not None and not source_frame_gdf.empty:
        source_frame_gdf.to_crs(display_crs).plot(ax=ax, color="#dc2626", linewidth=0.8, alpha=0.75)
    if model_bbox_gdf is not None and not model_bbox_gdf.empty:
        model_bbox_gdf.to_crs(display_crs).boundary.plot(
