...
- On the Adaptive-v2 Gmsh path, validate the exact delivered OBC segment membership on the unchanged chorded exterior, including complete one-time coverage, contiguity, cyclicity, IDs, node counts, and hard-anchor counts. Keep any smooth-source-curve versus delivered-chord overlap fraction diagnostic-only; never activate a numerical gate by scanning descriptive `required_revalidation`, `build_policy`, or case-name text.
...
- Resolve `--conditioning-profile auto` to `minimal-topology-v1`. Run only fixed-boundary valence repair, immediate superthin cleanup, residual protected-edge-safe superthin repair, and a terminal valence/thin scan. Preserve absolute structural invariants; prioritize valence debt, then superthin debt; never roll either repair back because of Class-2 angle, quality, area-transition, slope, singly-connected-count, or size-continuity changes. Do not run spring relaxation, area-transition relaxation, pruning, general boundary edits, global retriangulation, or the broad legacy postprocessor implicitly. The sole boundary-discretization exception is the audited non-OBC one-triangle fan repair below. It covers either one all-fixed/all-hard chain or a strictly sub-resolution two-chain passage with exactly one unambiguous source-chain edge; it adds that edge's exact midpoint without moving or removing an original boundary node.
- Keep `autonomous-thin-v1` opt-in. After minimal conditioning, have Codex inspect hash-bound whole-domain and component diagrams, classify the causal mechanism, and route it to bounded local repair, resolution refinement, localized GSHHS boundary regularization, or complete subgrid-connection closure followed by full Gmsh-6 remeshing. GSHHS is the only coastline source in this workflow; do not invoke CUSP or another shoreline skill. Do not change `auto` until forward testing is reviewed, and never insert a routine human-review gate or delete one triangle in isolation. Read `references/autonomous_thin_boundary.md` before using this profile.
...
3. Repair residual connected superthin debt with protected-edge-safe flips, collapses, or bounded local cavity reconstruction. For a one-triangle `fixed-boundary-hard-anchor-fan` on exactly one non-OBC chain, automatically test one zero-support source-arc midpoint reconstruction through the `1 -> 2 -> 4` ring ladder. Commit the first passing smallest patch only when superthin debt strictly falls, maximum valence remains at most eight, original boundary coordinates and hard anchors are exact, OBC membership/order is unchanged, and every structural/constraint invariant passes. Do not request visual review for this fully audited class. Never delete a triangle in isolation or remove a wet passage.
...
6. When `systematic-v2` or `systematic-v3` is selected, repeat it once after area-transition conditioning as terminal thin-debt closure. When V5 is selected, first establish a zero-superthin and zero-restricted-edge champion, then run bounded fixed-connectivity edge-angle-barrier bursts and the complete connectivity-restricted V5 closure after each selected checkpoint; compare quality only between zero-debt states and always finish with closure. V6 preserves the order `connectivity restriction → locked-star/multi-support repair → component cavity → connectivity recheck → terminal locked-star → optional authorized passage sweep → valence repair → immediate thin cleanup → second optional passage sweep → global audit`. Do not begin relaxation while superthin, valence-above-eight, or restricted-edge debt remains. On the standalone thin-repair interface, `--no-systematic-v5-boundary-window-fallback` makes every V5/V6 topology-preserving transaction boundary- and OBC-membership-immutable; it is not an integrated `run_fvcom_grid.py` option. V2 requires exact surviving boundary coordinates. V3/V5/V6 require zero source-arc normal offset, exact hard anchors, simple loops, passage-clearance loss no larger than 0.5 m, and a valid OBC remap. Roll back a complete relaxation/closure cycle unless it returns to zero debt, adds no singly connected or boundary-anomaly elements, and improves `q_l3_sigma` by at least `1e-4`.
   ...
   - Apply the plan with `apply_visual_superthin_plan.py`. The plan is bound to the exact input SHA-256, names a bounded topology-tool sequence, and commits at most one globally audited transaction. Class-2/3 changes in `q_min`, `q_p01`, `q_l3_sigma`, ordinary angle tails, `L/h`, and adjacent-area transitions are recorded as advisories and cannot veto a transaction that strictly reduces superthin debt while passing every exact structural, constraint, boundary-coordinate, hard-anchor, and OBC-lineage safeguard. Regenerate the component atlas after every accepted transaction before planning the next component because connectivity and component identifiers may change.
...
- `scripts/diagnose_autonomous_thin.py`: create the whole-domain locator, local mesh/boundary and lineage diagrams, GSHHS overlay, scale evidence, and one pending Codex decision per connected superthin component.
...
- `scripts/condition_mesh_local.py`: apply the complete `aggressive-local-v2` protocol to an existing 2DM, with optional adaptive boundary metadata and size field; `--fixed-hard-fan-arc-refinement` reproduces the default minimal profile's bounded non-OBC fan transaction for an isolated conditioning proof.
...
- `scripts/diagnose_autonomous_thin.py`: extend the atlas with a complete-domain locator, boundary-lineage table, scale-derived GSHHS context window, GSHHS overlay, and pending hash-bound Codex decision documents.
...
python scripts/selftest_adaptive_open_delivery.py
