...
one pending decision, then invoke the same command with `--decision`. Regenerate
the diagnostic against the same hash-bound GSHHS package before completing a
shoreline-correction decision. Do not invoke CUSP in this workflow. Use
`--execute` only after all hashes and source contracts are present.
...
- Derive the local GSHHS context buffer as
  `clip(max(10*h, 2*component_diameter, 1 km), 1 km, 5 km)`.
- Keep GSHHS as the closed topology scaffold and the only coastline source.
- Rank GSHHS arcs by bracket coverage, endpoint distance, tangent agreement,
  available source metadata, and geometric validity. Reject a
  regularized candidate when either bracket junction turns more than 135°.
  Simplify at `max(available_horizontal_accuracy, 0.25*h)` and resample at
  model scale.
...
- Replace an acute spike/sliver with the best eligible regularized GSHHS arc or,
