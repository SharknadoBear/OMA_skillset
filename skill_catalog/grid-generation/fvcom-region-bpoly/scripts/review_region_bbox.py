...
def main() -> None:
    ap = argparse.ArgumentParser(description="Review a RegionBox candidate.")
    ...
    if args.decision == "pass":
        ...
        if region.crosses_antimeridian() and args.map_visibility_status != "pass":
        ...
        if args.domain_type_note_json:
            note = read_json(args.domain_type_note_json)
        ...
        if blocks:
            raise SystemExit("Cannot pass RegionBox: " + "; ".join(blocks))
    ...
    if args.decision == "pass":
        ...
        out = write_json(run_dir / f"{name}_region_box.json", final)
        # Also write convenient generic alias for subagent workflows.
        write_json(run_dir / "region_box.json", final)
        print(f"Wrote accepted RegionBox: {out}")
    else:
