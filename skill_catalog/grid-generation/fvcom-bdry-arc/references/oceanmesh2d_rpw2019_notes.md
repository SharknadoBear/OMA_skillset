...
Repair a coastal OBC against GSHHS land with fixed anchors before assigning nodes. Grade target spacing by arclength and build one continuous chain rather than sampling each source segment independently. Split a graded interval when its chord deviates from the repaired polyline by more than ten percent of local target size. Then bisect every sampled chord whose non-landfall interior intersects physical land or a nonadjacent sampled chord, placing all added nodes on the unchanged exact source OBC; for a closed loop, apply this guard to the complete chain with no endpoint exception and inventory crossing candidates with a spatial index. Retain the iteration count, added-node count, unresolved land-intersection metrics, nonadjacent crossing count, and final simplicity state. Validate anchor equality, land avoidance for repaired and sampled paths, measured exterior overlap, chain order, and local `L/h` before downstream meshing.
