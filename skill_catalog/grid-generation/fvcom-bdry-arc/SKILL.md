...
- After model-loop construction, complete the RegionBPoly boundary-completeness loop before residual-role assignment. Two nontrivial cuts on one non-OBC side, required-feature/connectivity truncation, or a longitudinal frame-supported bar must move the polygon outward. A single ambiguous cut receives a hash-bound Codex decision; an isolated transverse intentional closure may proceed directly. Never let a solid role hide a suspected truncation.
...
The loop writes `region_bpoly_arc_feedback_v2` and `region_bpoly_arc_feedback_loop_v2`. It searches 1 km outward steps refined to 250 m, keeps at most three full arc candidates per iteration and four monotonic adjustments, and caps cumulative side motion at 100 km. Every retained adjustment map must have `geography_usable=true`. If the loop stops at `agent_decision_required`, inspect the whole/component maps, run `finalize_boundary_completeness_decision.py`, then resume the same loop with `--resume`. No human-review pause is used.
