...
- Require simple/nonbranching OBCs, exact physical land-polygon landfalls, no nonendpoint land crossing, one valid connected wet component, a continuous exterior ring, current source hashes, and the requested OBC count.
...
- Before Adaptive-v2 arc refinement, place each provisional coastal OBC endpoint at the first exact crossing with the physical land-polygon boundary. When the endpoint begins in water, extend its outward terminal ray and append the first land crossing. When the endpoint begins inside land, follow the existing source chain waterward, discard the land-interior tail, and prepend or append its first exact boundary exit. This is a geometric construction with no landfall-distance acceptance tolerance. Keep every added crossing on the source terminal linework so refinement redistributes the complete delivered chain without creating an isolated short edge or artificial feature anchor. If the required ray or source-chain exit does not intersect physical land, return a typed blocker; never substitute a nearest shoreline point or a RegionBPoly-side projection.
- Treat RegionBPoly-side points only as provisional OBC construction references. They are not delivered landfalls. Record the original references, exact landfall coordinates, extension lengths, and construction stage in lineage.
