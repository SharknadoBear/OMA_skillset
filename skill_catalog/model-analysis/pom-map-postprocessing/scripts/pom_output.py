"""Load, validate, concatenate, and derive scalar fields from POM NetCDF output."""
...
MASK_NAMES = ("mask", "wet", "wet_mask", "mask_rho")
...
class POMGrid:
    """Canonical native POM curvilinear grid."""
...
def _geometry_hash(
    ...
    digest = hashlib.sha256()
    for name, array in (("lon", lon), ("lat", lat), ("mask", mask), ("depth", depth), ("sigma", sigma)):
...
def read_grid(ds: Dataset) -> POMGrid:
    """Read a canonical POM grid from an open dataset."""

    lon_name = _find_name(ds, LON_NAMES)
    lat_name = _find_name(ds, LAT_NAMES)
    if lon_name is None or lat_name is None:
        raise KeyError("POM input must contain two-dimensional lon and lat variables.")
    ...
    mask_name = _find_name(ds, MASK_NAMES)
    mask = _to_2d(ds.variables[mask_name], mask_name) if mask_name else np.ones_like(lon)
    if mask.shape != lon.shape:
        raise ValueError(f"Mask shape {mask.shape} differs from coordinate shape {lon.shape}.")
    mask = np.where(np.isfinite(mask) & (mask == 1), 1.0, 0.0)
    ...
    if sigma_name:
        ...
        if sigma.size > 1:
            ...
            if not (np.all(delta > 0) or np.all(delta < 0)):
                raise ValueError("Sigma coordinate must be strictly monotonic.")
    ...
    return POMGrid(
        ...
        depth=depth,
        sigma=sigma,
        geometry_sha256=_geometry_hash(lon, lat, mask, depth, sigma),
    )


def _assert_same_grid(reference: POMGrid, candidate: POMGrid, path: Path) -> None:
    if reference.shape != candidate.shape:
        raise ValueError(f"POM grid shape drift in {path}: {candidate.shape} versus {reference.shape}.")
    checks = (
    ...
    for name, left, right, atol in checks:
        if (left is None) != (right is None):
            raise ValueError(f"POM {name} schema drift in {path}.")
        if left is not None and not np.allclose(left, right, rtol=0.0, atol=atol, equal_nan=True):
            raise ValueError(f"POM {name} geometry drift in {path}.")
...
def decode_times(ds: Dataset) -> tuple[np.ndarray, str]:
    """Decode raw POM timestamps without cadence snapping."""

    time_name = _find_name(ds, TIME_NAMES)
    if time_name is None:
        raise KeyError("POM input has no recognized time variable.")
...
def normalize_times(
    ...
    *,
    ...
    """Snap near-hourly or near-six-minute records to a UTC cadence."""
    ...
    snapped_ns = np.rint(ns.astype(np.float64) / cadence_ns).astype("int64") * cadence_ns
...
def sigma_weights(sigma: np.ndarray) -> np.ndarray:
    """Return trapezoidal sigma-point weights normalized to one."""

    levels = np.asarray(sigma, dtype=float).reshape(-1)
    if levels.size < 2 or not np.isfinite(levels).all():
        raise ValueError("Depth averaging requires at least two finite sigma points.")
    spacing = np.abs(np.diff(levels))
    if np.any(spacing <= 0):
        raise ValueError("Depth averaging requires a strictly monotonic sigma coordinate.")
...
def resolve_layer_index(sigma: np.ndarray | None, layer: str) -> int:
    ...
    order = np.argsort(np.abs(np.asarray(sigma, dtype=float)))
    if selector == "surface":
        return int(order[0])
...
def _has_earth_components(ds: Dataset, east_name: str, north_name: str, *, wind: bool = False) -> None:
    ...
    raise ValueError(
        f"Cannot prove {east_name!r}/{north_name!r} are earth-relative; add CF standard_name metadata."
    )
...
def load_scalar_series(
    ...
    *,
    ...
    """Load, grid-check, normalize, deduplicate, and crop scalar POM records."""
...
def inspect_inputs(inputs: Sequence[str | Path]) -> dict[str, Any]:
    """Return JSON-ready metadata and geometry checks for POM inputs."""
