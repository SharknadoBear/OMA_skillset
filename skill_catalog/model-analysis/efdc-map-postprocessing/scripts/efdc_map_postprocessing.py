"""CLI for inspecting and mapping EFDC curvilinear NetCDF fields."""
...
try:
    from .efdc_map_tools import colormap_for_variable, quantile_limits, save_efdc_scalar_map
    from .efdc_output import inspect_inputs, load_scalar_series, parse_time, validate_vector_components
...
def command_inspect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if Path(args.output).resolve() in {item.resolve() for item in sources}:
        raise ValueError("Inspection output must not overwrite the NetCDF input.")
    report = inspect_inputs(sources)
    report["schema_version"] = "efdc_map_inspection_v1"
...
def _source_field_metadata(path: Path, source_variables: Sequence[str], requested: str) -> tuple[str, str]:
    with Dataset(path) as ds:
        variables = [ds.variables[name] for name in source_variables if name in ds.variables]
        if not variables:
            return "", ""
        units = str(getattr(variables[0], "units", ""))
        aliases = {"salinity": "Practical salinity", "current_speed": "Current speed", "wind_speed": "Wind speed"}
        long_name = aliases.get(requested.lower(), str(getattr(variables[0], "long_name", source_variables[0])))
...
def command_map(args: argparse.Namespace) -> dict[str, Any]:
    ...
    resolved_inputs = {item.resolve() for item in sources}
    if output.resolve() in resolved_inputs or report_path.resolve() in resolved_inputs or output.resolve() == report_path.resolve():
        raise ValueError("Inputs, --output, and --report must be different paths.")
    if output.suffix.lower() != ".png":
        raise ValueError("--output must use a .png extension.")

    series = load_scalar_series(sources, variable=args.variable, layer=args.layer)
    ...
    if args.quiver != "none":
        ...
        u_series = load_scalar_series(sources, variable=first, layer=vector_layer)
        v_series = load_scalar_series(sources, variable=second, layer=vector_layer)
        for vector_source in sources:
    ...
    grid = "sjrofs_native"
    title = args.title or f"SJROFS {args.variable} ({args.layer})\n{time_label}"
    selected_source = Path(series.record_sources[index])
    units, long_name = _source_field_metadata(selected_source, series.resolution["source_variables"], args.variable)
    ...
    manifest: dict[str, Any] = {
        "schema_version": "efdc_map_manifest_v1",
        "status": "pass",
        "created_utc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "inputs": [_input_provenance(item) for item in sources],
        "request": {
        ...
        "selection": {
            ...
            "source_record_path": str(selected_source.resolve()),
        ...
        "rendering": {
            ...
            "rendering_method": result.rendering_method,
            "rendered_cell_count": result.rendered_cell_count,
            "footprint_fallback_cell_count": result.footprint_fallback_cell_count,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="efdc_map_postprocessing.py",
        description="Inspect and render native-grid EFDC curvilinear NetCDF fields.",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    inspect_parser = subparsers.add_parser("inspect", help="Inventory grid, time, sigma, and variables.")
    inspect_parser.add_argument("--input", action="append", required=True, help="EFDC NetCDF input; repeat as needed.")
    ...
    map_parser.add_argument("--input", action="append", required=True, help="Raw or compact EFDC NetCDF input; repeat as needed.")
    ...
    map_parser.add_argument("--style", choices=("wet_cells", "pcolormesh", "contourf"), default="wet_cells")
