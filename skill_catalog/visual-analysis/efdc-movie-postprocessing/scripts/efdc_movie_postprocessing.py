#!/usr/bin/env python3
"""Inspect EFDC files and render fixed-color-scale scalar GIFs."""
...
try:
    from .efdc_map_tools import build_wet_cell_footprints, colormap_for_variable, plot_efdc_scalar, quantile_limits
    from .efdc_output import inspect_inputs as inspect_efdc_inputs
    from .efdc_output import load_scalar_series
...
MANIFEST_VERSION = "efdc_movie_manifest_v1"
...
def inspect_inputs(inputs: Sequence[str | Path]) -> dict[str, Any]:
    """Inspect EFDC inputs without loading a movie into memory."""

    paths = _flatten_inputs(inputs)
    report = inspect_efdc_inputs(paths)
    return {
        "schema_version": "efdc_movie_inspection_v1",
...
def create_gif(
    ...
    *,
    ...
    """Create a fixed-scale EFDC GIF and return its provenance manifest."""
    ...
    if wet_count == 0:
        raise ValueError("The EFDC grid contains no finite cells with mask == 1.")
    coverage = np.asarray(
    ...
    footprints = build_wet_cell_footprints(lon, lat, mask)
    ...
    if output_path.suffix.lower() != ".gif":
        raise ValueError("--output must use a .gif extension.")
    collision_paths = {path.resolve() for path in paths}
    if output_path in collision_paths or (report is not None and Path(report).expanduser().resolve() in collision_paths):
        raise ValueError("Inputs, --output, and --report must be different paths.")
    if report is not None and output_path == Path(report).expanduser().resolve():
    ...
    staged_output = output_path.with_name(output_path.stem + ".tmp" + output_path.suffix)
    gif_frame_count = 0
    gif_size: list[int] = []
    gif_duration = duration_ms
    try:
        with tempfile.TemporaryDirectory(prefix="efdc_movie_") as tmp_name:
            ...
            with Image.open(staged_output) as gif:
                ...
                if gif_frame_count != len(times):
                    raise RuntimeError(
                        f"Staged GIF contains {gif_frame_count} frames; expected {len(times)}."
                    )
    ...
    manifest: dict[str, Any] = {
        ...
        "rendering": {
            ...
            "movie_quivers": False,
            "style": "wet_cells",
            "wet_footprint_method": "efdc_immediate_wet_neighbor_centers",
            "wet_footprint_count": int(len(footprints.polygons)),
            "wet_footprint_fallback_cell_count": int(np.count_nonzero(footprints.fallback_cells)),
            "wet_footprint_maximum_span_km": float(footprints.maximum_span_km),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect and animate scalar fields on NOAA EFDC curvilinear grids."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    inspect_parser = subparsers.add_parser("inspect", help="Inspect EFDC inputs and write JSON metadata.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "inspect":
        ...
        if inspection_output in {path.resolve() for path in inputs}:
            raise ValueError("Inspection output must not overwrite a NetCDF input.")
        payload = inspect_inputs(inputs)
        path = _write_json(inspection_output, payload)
